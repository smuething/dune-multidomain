...
public:
  ...
  template<int sign,
           ...
           typename PowerLFSU_C, typename PowerLFSV_C, typename R>
  void alpha_generic
  ( const IG& ig,
    const LFSU& lfsu, const X& x, const LFSV& lfsv,
    const PowerLFSU_C& powerlfsu_c, const X& x_c, const PowerLFSV_C& powerlfsv_c,
    R& r, R& r_c) const
  {
    ...
    typedef typename PowerLFSU_C::template Child<0>::Type LFSU_C;
    typedef typename PowerLFSV_C::template Child<0>::Type LFSV_C;

    for (std::size_t c_block = 0; c_block < PowerLFSU_C::CHILDREN; ++c_block)
      {
        const LFSU_C& lfsu_c = powerlfsu_c.child(c_block);
        const LFSV_C& lfsv_c = powerlfsv_c.child(c_block);
